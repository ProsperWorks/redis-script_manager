...
class Redis
  class ScriptManagerTest < Minitest::Test

    # When ENV['REDIS_URL'] is set, we run a greatly expanded test
    # suite which actually talk to Redis.
    #
    ...
    def setup
      Redis::ScriptManager.configuration =
        Redis::ScriptManager::Configuration.new
    ...
    def test_minify_lua
      ...
      #
      raw = "return 'careful -- not to delete this' --  but this can go   \n" +
            "return \"ditto -- preserve this piece\" --  but please cut this \n"
      min = "return 'careful -- not to delete this'\n" +
      ...
      raw = "if true then\n" +
            "  --\n" +
            "  -- Avoids the \"too many results to unpack\" problem, but\n" +
            ...
            "    num_rem = num_rem + redis.call('ZREM',ick_cset_key,v)\n" +
            "  end\n" +
            "  return num_rem\n" +
            "else\n" +
            "  --\n" +
            "  -- Produces the \"too many results to unpack\" w/ big ARGV\n" +
            "  --\n" +
    ...
    [
      ...
      define_method("test_EVAL_EVALSHA_and_SCRIPT_LOAD_for_#{test_name}") do
        next if !redis
        #
        # Clean slate:
        #
        ...
        #
        assert_equal 13, redis_to_test.eval('return 13',['k'])          # 1 key
        assert_equal 13, redis_to_test.eval('return 13',['a{k}','b{k}'])# 1 slot
        #
        # We can pass args to a simple inline Lua snippet, results are stringy.
        #
        assert_equal ['1','2'], redis_to_test.eval('return ARGV',[],[1,2])
        #
    ...
    [
      ...
      define_method("test_eval_lua_in_redis_do_preload_#{do_preload}") do
        ...
        statsd                                        = MockStatsd.new
        Redis::ScriptManager.configuration.statsd     = statsd
        #
        # Clean slate:
        #
        big = "x" * Redis::ScriptManager.configuration.max_tiny_lua
        #
        # Legit scripts:
        #
        [
        ...
        assert_equal expect_stats, statsd.flush
        #
        # And again with another bogus script over the magic threshold.
        #
        assert_raises(Redis::CommandError) do                             # big
          Redis::ScriptManager.eval_gently(redis,"-- #{big}\nrexxyv",['k'])
        end
        expect_stats = if do_preload
                         [
                           [:increment, "preloaded_shas.cache_miss"],
    ...
    def test_implications_of_pipelined
      ...
      s_str_a         = 'x' * (max_tiny_lua/10)
      l_str_a         = 'x' * max_tiny_lua
      s_lua_a         = "return '#{s_str_a}'" # not cached, will eval
      l_lua_a         = "return '#{l_str_a}'"  # not cached, will evalsha
      s_got_a         = Redis::ScriptManager.eval_gently(redis,s_lua_a,['k'])
      l_got_a         = Redis::ScriptManager.eval_gently(redis,l_lua_a,['k'])
      assert_equal s_str_a, s_got_a
      assert_equal l_str_a, l_got_a
      #
      # Try the same thing in a pipeline, only the short stuff works.
      #
      s_str_b         = 'y' * (max_tiny_lua/10)
      l_str_b         = 'y' * max_tiny_lua
      s_lua_b         = "return '#{s_str_b}'" # not cached, will eval
      ...
      #
      assert_equal l_str_b, l_got_b.value   # fails w/o in_pipeline check
      #
    ...
    def test_implications_of_do_preload
      ...
      Redis::ScriptManager.purge_preloaded_shas
      max_tiny_lua    = Redis::ScriptManager.configuration.max_tiny_lua
      s_str_a         = 'a' * (max_tiny_lua/10)
      l_str_a         = 'a' * max_tiny_lua
      s_lua_a         = "return '#{s_str_a}'" # not cached, will eval
      l_lua_a         = "return '#{l_str_a}'"  # not cached, will evalsha
      s_got_a         = Redis::ScriptManager.eval_gently(redis,s_lua_a,['k'])
      l_got_a         = Redis::ScriptManager.eval_gently(redis,l_lua_a,['k'])
      ...
      #
      s_str_b         = 'b' * (max_tiny_lua/10)
      l_str_b         = 'b' * max_tiny_lua
      s_lua_b         = "return '#{s_str_b}'" # not cached, will eval
      l_lua_b         = "return '#{l_str_b}'"  # not cached, will evalsha
      s_got_b         = nil
      l_got_b         = nil
